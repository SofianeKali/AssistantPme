...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const evolution = await storage.getTaskEvolutionByWeek(
          offset,
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch task evolution data" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const offset = parseInt(req.query.offset as string) || 0;
        const periodType = (req.query.periodType as string) || "week";
        const evolution = await storage.getAlertEvolutionByWeek(
          offset,
          periodType as "week" | "month",
          companyId,
        );
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch alert evolution data" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const offset = parseInt(req.query.offset as string) || 0;
        const periodType = (req.query.periodType as string) || "week";
        const appointments = await storage.getAppointmentsByWeek(
          offset,
          periodType as "week" | "month",
          companyId,
        );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const periodType = (req.query.periodType as string) || "week";
        const emailAccountId = req.query.emailAccountId as string | undefined;
        const distribution = await storage.getEmailDistribution(
          offset,
          periodType as "week" | "month",
          emailAccountId,
          companyId,
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch email distribution data" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const periodType = (req.query.periodType as string) || "week";
        const emailAccountId = req.query.emailAccountId as string | undefined;
        const evolution = await storage.getEmailEvolution(
          offset,
          periodType as "week" | "month",
          emailAccountId,
          companyId,
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch email evolution data" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const periodType = (req.query.periodType as string) || "week";
        const emailAccountId = req.query.emailAccountId as string | undefined;
        const processing = await storage.getCategoryProcessing(
          offset,
          periodType as "week" | "month",
          emailAccountId,
          companyId,
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch category processing data" });
      }
    },
  );
  ...
  app.post("/api/dashboard/layout", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const savedLayout = await storage.upsertUserDashboardLayout(
        userId,
        layout,
      );
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        const updated = await storage.updateEmailAccount(
          req.params.id,
          req.body,
        );
        res.json(updated);
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        console.log(
          `[Manual Scan] Starting scan for account: ${account.email}`,
        );
        const result = await emailScanner.scanAccount(account);

        console.log(
          `[Manual Scan] Scan complete - Scanned: ${result.scanned}, Created: ${result.created}, Errors: ${result.errors}`,
        );
  ...
  app.get(
    "/api/email-accounts/:id/categories",
    isAuthenticated,
    async (req, res) => {
      try {
    ...
    async (req, res) => {
      try {
        ...
        if (!Array.isArray(categoryIds)) {
          return res
            .status(400)
            .json({ message: "categoryIds must be an array" });
        }
        ...
        res
          .status(500)
          .json({ message: "Failed to update account categories" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch email category stats" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        console.log(
          `[AI Search] Analyzed prompt: "${prompt}" (deep: ${deepSearch}) →`,
          criteria,
        );
  ...
  app.post("/api/emails/ai-search", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!criteria || typeof criteria !== "object") {
        return res
          .status(400)
          .json({ message: "Search criteria are required" });
      }
      ...
      console.log(
        `[AI Search] Executed search with ${Object.keys(criteria).length} criteria (deep: ${deepSearch}) → ${result.total} results`,
      );
  ...
  app.post(
    "/api/emails/:id/generate-response",
    isAuthenticated,
    async (req: any, res) => {
      try {
  ...
  app.post(
          ...
          if (err instanceof multer.MulterError) {
            if (err.code === "LIMIT_FILE_SIZE") {
              return res.status(413).json({
                ...
                error:
                  "Un ou plusieurs fichiers dépassent la taille maximale de 15 MB",
              });
            }
    ...
    async (req: any, res) => {
      try {
        ...
        const emailAccount = await storage.getEmailAccountById(
        ...
        console.log(
          `[Email Response] Sending response with ${attachments.length} attachment(s)`,
        );
    ...
    async (req: any, res) => {
      try {
        ...
        if (!Array.isArray(emailIds) || emailIds.length === 0) {
          return res
            .status(400)
            .json({ message: "Email IDs array is required" });
        }
  ...
  app.patch(
    "/api/emails/bulk/mark-processed",
    isAuthenticated,
    async (req: any, res) => {
      try {
  ...
  app.patch(
    "/api/email-responses/:id/approve",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = (req.user as any).id;
    ...
    async (req, res) => {
      try {
        ...
        if (!response.isApproved) {
          return res
            .status(400)
            .json({ message: "Response must be approved before sending" });
        }

        // TODO: Implement actual email sending via SMTP
        // For now, just mark as sent
        const updatedResponse = await storage.updateEmailResponse(
          req.params.id,
          {
            isSent: true,
            sentAt: new Date(),
  ...
  app.get(
    "/api/emails/action-recommendations",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = (req.user as any).id;
        ...
        const recommendations = allEmails
          .filter((email) => {
            const aiAnalysis = email.aiAnalysis as any;
            return (
              aiAnalysis?.actionRecommendations &&
              aiAnalysis.actionRecommendations.length > 0
            );
          })
          .map((email) => {
            const aiAnalysis = email.aiAnalysis as any;
        ...
        recommendations.sort((a, b) => {
          ...
          const aMaxPriority = Math.max(
            ...a.recommendations.map(
              (r: any) =>
                priorityRank[r.priority as keyof typeof priorityRank] || 0,
            ),
          );
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch action recommendations" });
      }
    },
  );
  ...
  app.patch(
    "/api/reminders/:id/mark-response-received",
    isAuthenticated,
    async (req, res) => {
      try {
        const updatedReminder = await storage.updateReminder(req.params.id, {
  ...
  app.get("/api/documents/:id/download", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!document.emailId) {
        return res
          .status(400)
          .json({ message: "Document not associated with any email" });
      }
  ...
  app.post("/api/documents/:id/ocr", isAuthenticated, async (req, res) => {
    try {
      ...
      console.log(
        `[OCR] Processed document ${document.id}: ${result.text.length} chars extracted via ${result.method}`,
      );
  ...
  app.get("/api/alerts/:id/emails", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (emailIds.length > 0 && emails.length === 0) {
        return res
          .status(403)
          .json({ message: "Accès non autorisé à cette alerte" });
      }
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(400)
          .json({
            message: error.message || "Impossible de créer la règle d'alerte",
          });
      }
    },
  );
    ...
    async (req, res) => {
      try {
        ...
        if (!rule) {
          return res
            .status(404)
            .json({ message: "Règle d'alerte non trouvée" });
        }
        res.json(rule);
      } catch (error) {
    ...
    async (req, res) => {
      try {
        ...
        if (updateData.prompt) {
          const { interpretAlertPrompt } = await import("./openai");
          const { name, ruleData, severity } = await interpretAlertPrompt(
            updateData.prompt,
          );
        ...
        res
          .status(400)
          .json({ message: error.message || "Failed to update alert rule" });
      }
    },
  );
  ...
  app.patch(
    "/api/email-categories/:id",
    isAuthenticated,
    isAdmin,
    async (req, res) => {
      try {
        const updated = await storage.updateEmailCategory(
          req.params.id,
          req.body,
        );
        res.json(updated);
    ...
    async (req, res) => {
      try {
        ...
        if (toDelete?.isSystem) {
          return res
            .status(400)
            .json({ message: "Cannot delete system category" });
        }
        ...
        if (toDelete?.key === "autre") {
          return res
            ...
            .json({
              message:
                "Cannot delete the 'autre' category - it is required as a fallback",
            });
        }
  ...
  app.get("/api/cloud-storage-configs", isAuthenticated, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch cloud storage configurations" });
    }
  });
  ...
  app.post("/api/cloud-storage-configs", isAuthenticated, async (req, res) => {
    try {
      ...
      const existingConfig = await storage.getCloudStorageConfig(
        userId,
      ...
      res
        .status(400)
        .json({ message: "Invalid cloud storage configuration data" });
    }
  });

  app.patch(
    "/api/cloud-storage-configs/:id",
    isAuthenticated,
    async (req, res) => {
      try {
        const userId = req.user!.id;
        ...
        if (!configToUpdate) {
          return res
            .status(404)
            .json({ message: "Cloud storage configuration not found" });
        }

        const updated = await storage.updateCloudStorageConfig(
          configId,
          req.body,
        );
        res.json(updated);
        ...
        res
          .status(500)
          .json({ message: "Failed to update cloud storage configuration" });
      }
    },
  );
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to delete cloud storage configuration" });
      }
    },
  );
  ...
  app.post("/api/email-scan", isAuthenticated, async (req, res) => {
    try {
      ...
      res.json({
        ...
        results,
        summary: {
          totalScanned: Object.values(results).reduce(
            (sum, r) => sum + r.scanned,
            0,
          ),
          totalCreated: Object.values(results).reduce(
            (sum, r) => sum + r.created,
            0,
          ),
      ...
      res
        .status(500)
        .json({ message: "Failed to scan emails", error: String(error) });
    }
  });
  ...
  app.post("/api/email-scan/:accountId", isAuthenticated, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          message: "Failed to scan email account",
          error: String(error),
        });
    }
  });
  ...
  app.post("/api/users", isAuthenticated, isAdmin, async (req, res) => {
    try {
      const { sendWelcomeEmail: sendEmail, generateTemporaryPassword } =
      ...
      console.log(
        `[API] Generated temporary password for ${normalizedEmail} (length: ${temporaryPassword.length})`,
      );
      console.log(
        `[API] Password preview (first 4 chars): ${temporaryPassword.substring(0, 4)}...`,
      );
      ...
      try {
        ...
        if (!activeAccount) {
          console.warn(
            "[API] No active email account found for admin - skipping welcome email",
          );
        } else {
          await sendEmail({
            to: user.email!,
          ...
          console.log(
            `[API] Welcome email sent to ${user.email} via SMTP (${activeAccount.email})`,
          );
        }
      } catch (emailError) {
      ...
      try {
        ...
        if (userToDelete.role === "admin") {
          ...
          if (adminCount <= 1) {
            return res.status(400).json({
              message:
                "Impossible de supprimer le dernier administrateur. Il doit y avoir au moins un administrateur dans le système.",
            });
          }
        }
        ...
        console.log(
          `[API] User ${userToDelete.email} deleted by ${req.user.email}`,
        );
        res.json({
          success: true,
  ...
  if (!process.env.STRIPE_SECRET_KEY) {
    console.error(
      "[Stripe] Missing STRIPE_SECRET_KEY - payment routes will not be available",
    );
  }
  ...
  const PRICING_PLANS = {
    starter: {
      priceMonthly: 100, // €59.00 in cents
      name: "Starter",
      users: "1-5",
      emailsPerMonth: 500,
    },
  ...
  app.post("/api/create-subscription", async (req, res) => {
    ...
    try {
      const { plan, email, firstName, lastName, companyName, companyAddress } =
        req.body;
      ...
      if (
        ...
        return res
          ...
          .json({
            message:
              "Données manquantes (plan, email, firstName, lastName, companyName, companyAddress requis)",
          });
      }
  ...
  app.post("/api/start-trial", async (req, res) => {
    try {
      const { email, firstName, lastName, companyName, companyAddress } =
        req.body;
      ...
      if (
        ...
        return res
          ...
          .json({
            message:
              "Données manquantes (email, firstName, lastName, companyName, companyAddress requis)",
          });
      }
      ...
      console.log(
        `[Trial] Created trial user: ${email} for company ${company.id}`,
      );
      ...
      try {
        ...
        if (!defaultAccount) {
          console.warn(
            "[Trial] Default email account (kalizahir@yahoo.fr) not found or not active - skipping welcome email",
          );
        } else {
          ...
          console.log(
            `[Trial] Welcome email sent to ${email} via SMTP (${defaultAccount.email})`,
          );
        }
      } catch (emailError) {
